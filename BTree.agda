...
replaceTree1 {n} {A} {t} {t₁} k v1 value ti = tree-construct A t t₁ (proj1 (ti-property1 ti)) (proj2 (ti-property1 ti)) 
   (treeLeftDown t t₁ ti) (treeRightDown t t₁ ti)
...
RTtoTI0 {n} {A} tree repl key value ti1 rt1 = lem001 tree _ _ _ refl ti1 rt1 where
   lem001 : (tree tree1 : bt A) → (key : ℕ) → (value : A) → tree ≡ tree1 → treeInvariant tree  → replacedTree key value tree1 repl → treeInvariant repl
   lem001 tree _ key value eq ti r-leaf = t-single _ _
   lem001 tree _ key value eq ti (r-node {v1} {t} {t₁} ) = tree-construct _ _ _ lem03 lem04 (treeLeftDown _ _ (subst treeInvariant eq ti)) 
          (treeRightDown _ _ (subst treeInvariant eq ti)) where
      lem05 : treeInvariant (node _ v1 t t₁)
      lem05 = subst treeInvariant eq ti
      lem03 : tr< _ t
      lem03 = proj1 ( ti-property1 lem05) 
      lem04 : tr> _ t₁
      lem04 = proj2 ( ti-property1 lem05) 
   lem001 tree _ key value eq ti (r-right {k} {v1} {t} {t₁} {t₂} x rpl) = tree-construct _ _ _ lem03 lem04 
          (treeLeftDown _ _ lem05) lem02 where
      lem05 : treeInvariant (node k v1 t₁ t₂)
      lem05 = subst treeInvariant eq ti
      lem02 : treeInvariant t
      lem02 = RTtoTI0 _ _ _ _ (treeRightDown _ _ lem05) rpl
      lem03 : tr< k t₁
      lem03 = proj1 ( ti-property1  lem05)
      lem04 : tr> k t
      lem04 = ri-tr> _ _ _ _ _ rpl x (proj2 ( ti-property1  lem05))
   lem001 tree _ key value eq ti (r-left {k} {v1} {t} {t₁} {t₂} x rpl) = tree-construct _ _ _ lem03 lem04 
          lem02 (treeRightDown _ _ lem05) where
      lem05 : treeInvariant (node k v1 t₁ t₂)
      lem05 = subst treeInvariant eq ti
      lem02 : treeInvariant t
      lem02 = RTtoTI0 _ _ _ _ (treeLeftDown _ _ lem05) rpl
      lem03 : tr< k t
      lem03 = ri-tr< _ _ _ _ _ rpl x (proj1 ( ti-property1  lem05))
      lem04 : tr> k t₂
      lem04 = proj2 ( ti-property1  lem05) 
...
find-tree : {n m : Level} {A : Set n} {t : Set m} → (tree : bt A) → (key : ℕ) → (value : A) → treeInvariant tree
     → (exit : (tree1 : bt A) → (stack : List (bt A)) → treeInvariant tree1 ∧ stackInvariant key tree1 tree stack
                 → (tree1 ≡ leaf ) ∨ ( node-key tree1 ≡ just key )  → t ) → t
find-tree {n} {m} {A} {t} tree key value P0 exit =
...
-- insertTreeP-rt1 : {n m : Level} {A : Set n} {t : Set m} → (tree : bt A) → (key : ℕ) → (value : A) → treeInvariant tree
--      → replacedTree key value tree repl  → t
-- insertTreeP-rt1 = ?
